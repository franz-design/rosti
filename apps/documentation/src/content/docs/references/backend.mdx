...
There is no module generator. Copy an existing domain module such as `apps/api/src/modules/seasons/` and adapt it. That folder is a reference for entity, contract, mapper, service, controller, and module.

Then follow these steps (adapt as needed):

1. Create an entity, using an existing entity file (for example `season.entity.ts`) as reference;
2. Create a contract, using an existing contract file (for example `season.contract.ts`) as reference.
...
3. Create a mapper, using an existing mapper file as reference;
4. Create a service, using an existing service file as reference;
5. Create a controller, using an existing controller file as reference;
6. Create a module, using an existing module file as reference;
7. Add an e2e test under `tests/` when the module needs HTTP coverage. See [API testing](/lonestone-boilerplate/guides/api-testing) for how those tests run.
