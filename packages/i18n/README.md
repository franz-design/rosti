...
- `@rosti/i18n`: re-exports the main modules
- `@rosti/i18n/config`: supported locales, defaults, `initializeI18n`, `getHtmlLang`, `normalizeLocale`
- `@rosti/i18n/instance`: shared `i18next` singleton
- `@rosti/i18n/store`: `createI18nStore(i18n)` (localStorage + cookie mirror)
- `@rosti/i18n/utils`: `import.meta.glob` → i18next `Resource` helpers
- `@rosti/i18n/cookies`: cookie helpers + server `getLocaleFromRequestWithFallback`
